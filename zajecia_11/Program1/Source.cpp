...
// usuni趓ie definicji makr near i far
#ifdef near
#undef near
#endif
#ifdef far
...
void Keyboard(unsigned char key, int x, int y)// obsługa mgły
{
	// klawisz +
	if (key == '+')
		fog_density += 0.05;
...
void ExtensionSetup()
{
	...
	{
#ifdef WIN32
		printf("B喚dny format wersji OpenGL\n");
#else
...
int main(int argc, char * argv[])
{
...
#ifdef WIN32
	glutCreateWindow("Mg豉 odleg這iowa");
#else
...
#ifdef WIN32
	glutAddMenuEntry("Aspekt obrazu - ca貫 okno", FULL_WINDOW);
#else
	...
	glutCreateMenu(Menu);
#ifdef WIN32
	glutAddSubMenu("Rodzaj mg造", MenuFogMode);
